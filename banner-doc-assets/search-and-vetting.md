...
것은 JSON이 아니다.** `-o <file>`은 stdout을 거치지 않아 파일에 원본 바이트가 그대로 저장된다.

**"파이프가 문제"가 아니라 stdout 자체가 문제다.** 파이프(`| python3`)는 `JSONDecodeError`로
곧장 예외가 나지만, 셸 리다이렉트(`> out.json`)는 스키마 개요를 **조용히** 파일에 담아 나중
파싱에서야 예외가 나므로 원인을 찾기 어려워진다. 파이프를 리다이렉트로 바꾸는 "수정"은 증상만
감춘다.

RTK는 켜 둔 채로 쓴다. `-o`가 이미 우회하고 평범한 `curl -o`는 rtk가 없는 환경에서도 그대로
동작한다(훅은 rtk 부재 시 경고 후 통과시킨다). `-o`가 막히는 경우에만 문서화된 탈출구
