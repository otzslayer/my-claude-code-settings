...
| `extmetadata.Artist.value` | `banner_creator` | **HTML 태그 제거 후** 아래 쓰기 안전 규칙 적용. 개인 닉네임이나 `Photograph by …`면 **사진본 신호**(§ 정본 우선) |
...
| `imageinfo[0].url`의 파일명(원제) | `banner_title` | URL 마지막 세그먼트 → 확장자와 `File:`을 정리 |

### `extmetadata.Artist` HTML 평문화

`Artist.value`는 흔히 `<bdi><a href="…">이름</a></bdi>` 형태의 HTML이다. `banner_creator`에
넣기 전 **모든 HTML 태그를 제거해 평문화**한다(`<bdi>`와 `<a>` 등 흔적 없이). 이름만 남긴다.
...
이건 **미관 문제이지 아래 § 쓰기 안전(YAML 손상과 키 주입 방지)과 다른 사안**이다. 잔여물을
잘랐든 아니든 인용과 이스케이프는 그것대로 반드시 적용한다.
...
6개 `banner*` 스칼라에는 § 쓰기 안전(모든 소스 파생 값) 규칙을 동일하게 적용한다.
...
미 의회도서관 Prints & Photographs 온라인 카탈로그다. 보조 소스 중 유일하게 **완전 배선**돼 있다.
...
다운로드(해상도) 없이 하지 못한다. 즉 **탈락할 후보에 라운드트립을 먼저 지불하는 구조**다. 이것은
소스 품질 문제가 아니다. API 구조가 비대칭인 탓이고 § 보조 소스의 "직접 래스터 URL을 안정 추출 가능할
때만 보강한다"는 원칙과 근거가 같다.
...
- 최상위 **`unrestricted == true` 인 항목만 통과**시켜 `banner_license`에
  `No known restrictions (LoC)`로 기록한다. `false`이거나 필드가 없으면 **제외**한다(§ 감별 1: 저작권을
  독자적으로 판단하지 않고 소스 태그만 신뢰한다).
- `image.full`은 `.jpg`이므로 mime 필터를 자동으로 충족한다(§ 감별 2).
- 6개 `banner*` 스칼라에는 § 쓰기 안전(모든 소스 파생 값) 규칙을 동일하게 적용한다.
...
IIIF 뷰어와 아이템 페이지만 노출하고 직접 이미지 URL을 안정적으로 주지 않으므로 안 뽑히면 그
후보를 **스킵**한다. 개별 소스의 API 엔드포인트와 IIIF 매니페스트 파싱을 여기에 열거하지 않는다.
유지비만 늘고 실패율이 높다. 안정 URL 추출은 에이전트 재량의 best-effort이며 못 뽑으면 미련
없이 버린다. 1차 소스(Wikimedia와 LoC)만으로 최소 후보 수를 채우는 것이 정상 경로다.
...
Banner의 fetch는 이 조건을 못 붙이고 JS 챌린지도 못 풀어 **배너가 렌더되지 않음이 실측으로 확인**됐다
(서버측 이미지 프록시도 datacenter IP라 더 막힌다). 따라서 AIC는 1차와 보조 어디에서도 배너 소스로
...
기준 질의 `filetype:bitmap "genealogical tree" engraving`(71건)에 하나씩 더해 실측한 결과다.

| 시도 | 결과 | 판정 |
|---|---|---|
| `-incategory:"Files from Internet Archive Book Images Flickr stream"` | 64건 → 63건 | 카테고리명은 맞지만 효과가 1건. 쿼리만 길어진다 |
| `incategory:"Engravings"` | **0건** | `incategory`는 **직접 소속만** 본다. 상위 카테고리에는 하위 카테고리만 있고 직접 속한 파일이 없다 |
| `deepcategory:"Genealogical trees"` | **0건** | 이 엔드포인트에서 동작하지 않는다 |
| CMA `&type=Print` | **0건** (`geological` 9건 중) | 결과 수가 적은 소스에 하드 필터를 걸면 결과가 통째로 비어 버린다. `type`은 § CMA 반환 필드 매핑처럼 **시각 판정 힌트로만** 쓴다 |
