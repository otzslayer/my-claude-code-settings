...
### Step 3 — 검색 (Bash 2콜)
**1차 소스는 Wikimedia Commons 단일.** 자산 **§ 호출 레시피를 그대로 실행**한다:

1. **콜 1** — 전 컨셉을 한 번의 `Bash`로 fan-out, 각 결과를 스크래치패드에 **`-o wm_<컨셉>.json`**
   으로 저장. **`-o`는 필수다** — RTK 훅이 `curl`을 `rtk curl`로 재작성하고 `rtk curl`은 stdout에
   본문 대신 **스키마 개요**를 뱉으므로, 파이프(`| python`)도 리다이렉트(`> file`)도 JSON을 얻지
   못한다(자산 § RTK 상호작용). 인코딩은 **`%20`만**(`+`는 0건), 셸은 **zsh**(bash 연관배열 금지).
2. **콜 2** — 저장된 JSON 전부를 한 번에 파싱해 **라이선스·mime·해상도 필터를 적용한 컴팩트 표**만
   출력한다(후보당 2~3줄). raw JSON은 출력하지 않는다.
...
→ 해상도(`width` < 1000 배제) → 시각 판정**.

앞의 셋은 Step 3의 콜 2 파서가 이미 적용했으므로, 남은 일은 **시각 판정뿐**이다: 표에 남은
후보의 썸네일을 curl로 스크래치패드에 임시 저장 → `Read`로 실제 확인(볼트엔 저장 안 함).
다운로드 후 저해상으로 버리는 일이 없어야 한다 — 해상도는 이미 걸러졌다.

보는 순서는 **컨셉당 1장 → 해상도 순**이며, **통과 후보 2~3개가 모이면 멈춘다**. 시각 탈락은
정상이므로 탈락 시 대체 `Read`를 쓴다. `Read` 횟수에 **하드 상한은 걸지 않는다**(자산 § 감별 4 —
상한 때문에 최소 2개 미달로 재검색하면 더 비싸다).
...
- **라운드트립 예산**: 검색은 **Bash 2콜**(fan-out → 일괄 파싱), 컨셉 3개 × 검색어 1개. raw JSON을
  컨텍스트에 출력 금지(§ 호출 레시피). 단 시각 감별 `Read`에는 상한을 걸지 않는다 — 최소 2개
  확보가 절약보다 우선한다.
