...
1. **콜 1** — 3소스 × 전 컨셉을 한 번의 `Bash`로 fan-out해 스크래치패드에 `wm_<컨셉>.json`·
   ...
   `.obsidian/`을 찾아 정한다.
   - **모든 curl을 `&`로 띄우고 끝에서 `wait` 한 번**으로 받는다. 검색 요청은 서로 의존하지
     않는다. 실측에서 7개 요청이 **9.19초에서 2.00초**로 줄었다.
...
2. **콜 2** — 저장된 JSON 전부를 한 번에 파싱해 **라이선스·mime·해상도 필터와 중복 배제를 적용한
   컴팩트 표**만 출력한다(후보당 3~4줄). raw JSON은 출력하지 않는다. 표에는 6개 `banner*` 필드가
   `src`까지 전부 들어가므로, **선택 뒤에 JSON을 다시 파싱하지 않는다.**
...
- **다운로드는 `&`로 띄우고 `wait`로 받으며, 시트용 이미지만 축소한다**(Wikimedia `1280px-` →
  `500px-`, CMA `_print` → `_web`). `montage`가 각 칸을 420px로 줄이므로 화질 손실이 없다. 실측에서
  6장이 **8.97초·13MB에서 1.31초·2.3MB**로 줄었다. **배너로 기록하는 URL은 축소 전 원본**이다.
- **Wellcome URL은 축소하지 않는다.** `full/1024,`의 0바이트 응답이 이 소스의 해상도 게이트를
  겸해서, `400,`으로 낮추면 배너에 못 쓰는 항목이 시트를 통과한다.
...
- **라운드트립 예산**: 검색 2콜(3소스 fan-out → 일괄 파싱) + 시트 1콜 = **Bash 3콜**, 선택 후
  **0콜**. 컨셉 3개에 소스별 검색어 1개. raw JSON을 컨텍스트에 출력 금지(§ 호출 레시피). 시각
  감별은 **컨택트 시트 `Read` 1회**가 기본이고, 시트 재생성 횟수에는 상한을 걸지 않는다. 최소
  2개 확보가 절약보다 우선한다.
