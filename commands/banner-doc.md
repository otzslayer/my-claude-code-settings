...
4. **본문도 컨셉 재료로 쓴다.** 제목만으로 검색어를 짜면 뜻이 넓은 일반어가 되어 관련도가 떨어진다.
   1에서 이미 읽은 본문에서 고유 명사·시대·지역·되풀이되는 결을 건진다(자산 § 컨셉 도출).
...
1. **콜 1** — 3소스 × 전 컨셉을 한 번의 `Bash`로 fan-out해 스크래치패드에 `wm_<컨셉>.json`·
   ...
   - **관련도를 올리는 소스별 필터**: Wikimedia `gsrsearch`는
     **`filetype%3Abitmap%20filew%3A%3E999%20`로 시작**해 해상도 게이트를 쿼리로 올리고, Wellcome은
     **`workType=k,e`**(Pictures·Maps)로 좁힌다. 후자가 없으면 결과의 93%가 책 서지 레코드다.
...
- **후보 반복을 막는 장치는 셋이다** — 소스 다변화, 볼트·실행 내 중복 배제, 그리고 **관련도 밴드
  안에서만 섞기**. 파서는 `filter → dedup → index 정렬 → 상위 12개 밴드 → 밴드 셔플 → 5개 출력`
  순으로 돈다. 풀 전체를 섞으면 `gsrlimit=40`의 꼬리에 있는 무관한 결과가 상위와 같은 확률로
  올라온다. `gsrsort=random`은 관련도를 통째로 버리므로 쓰지 않는다.
- **관련도 순위를 버리지 않는다** — `generator=search`의 `query.pages`는 pageid로 키를 잡은
  딕셔너리라 순회 순서가 관련도 순이 **아니다**. 각 페이지의 **`index` 필드로 정렬**한 뒤 쓴다
  (자산 § 랭킹 고착을 푸는 법).
...
→ 해상도(폭 1000 미만 배제) → 중복 배제 → 관련도 정렬·밴드 → 시각 판정**.

앞 단계는 Step 3의 콜 2 파서가 이미 적용했으므로 남은 일은 **시각 판정뿐**이다. 후보를 한 장씩
