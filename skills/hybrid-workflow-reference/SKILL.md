...
description: Deferred reference sections of the Compound + Superpowers hybrid workflow — §2-§5 the full model recalibration table, complexity-scoring rationale, escalation/re-run gate and reviewer branching table, §6 unit granularity and serial-vs-parallel execution strategy for ce-plan/ce-work, §7 the 95% confidence brainstorming opener and rigor-probe lenses, §9 the memory and documentation tier table. Use when a model/effort routing call is non-obvious or contested, when grouping Implementation Units or choosing serial vs parallel subagents in ce-plan/ce-work, when opening a superpowers:brainstorming session, or when deciding which tier a spec, plan, solution, or memory file belongs to. The always-resident rules/hybrid-workflow.md holds §1, the §2-§5 routing quick card, §8, and §10.
---

# Hybrid workflow — deferred reference (§2–§5 · §6 · §7 · §9)
...
the source of truth for the pipeline; this one holds the stage-scoped sections
verbatim. Section numbering is unchanged.

For §2–§5 the resident file keeps a **routing quick card** that settles routine
calls on its own. Load this file for the reasoning behind the card: why the bands
resolve the way they do, the evidence for suspending sonnet, and the reviewer
split to restore if sonnet returns.
