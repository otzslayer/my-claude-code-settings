...
| 이벤트가 결과를 들고 온다 | 이벤트가 결과를 전한다, 결과가 실려 온다 |
| 실행 도중 흐르는 알갱이 | 실행 도중 흐르는 낱개 신호 |
| 상태 기계의 한 걸음 | 상태 기계의 한 단계 |
| 요청 머리에 꽂는다 | 요청 맨 앞에 넣는다 |
