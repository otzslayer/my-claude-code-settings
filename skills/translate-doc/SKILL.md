...
- 그 밖의 경로라면 **번역을 시작하기 전에 경고하고 사용자의 확인을 받는다.** 원문이 버전 관리 아래 있지 않으면 되돌릴 방법이 없다.
...
그 표를 읽고 `적용 위치` 칸이 **Step 4**인 패턴을 영어 원문과 대조하며 적용한다. `Step 5 (density)`인 것은 문장 단위가 아니라 문서 전체 빈도로 판단하므로 Step 5 전체 재독에서 점검한다.
...
`translationese-patterns.md`의 축 설명에는 `과감`이 남아 있으나 그런 등급은 이제 없다. 그 문서는 upstream 분류 체계를 추적하는 대장이라 손대지 않는다. **의역 강도의 정본은 이 문서이며 등급은 보수, 균형, 적극 셋뿐이다.**
