...
흡수 패턴은 위 Question 2에서 사용자가 고른 의역 강도와 두 등급으로 상호작용한다. translationese-patterns.md의 "패턴 인덱스" 표가 각 패턴의 등급을 ID별로 명시하며, 아래 분류와 1:1 일치한다:

- **표면 교정형** (C-11, A-7, E-2, F-4, A-19, PE15, G-1/G-2, H-3, A-3~A-6, A-11, A-13): **모든 의역 강도에서 적용한다.** 번역투 *오류 수정*이지 문체적 자유가 아니므로 보수에서도 고친다.
- **구조 재구성형** (A-15, A-16, A-18): **의역 강도에 단조 비례한다.** 문장 구조를 실제로 바꾸므로 보수에서는 명백한 경우만(3중+ 중첩 관형구, 단락 3회+ 대명사) 최소 적용하고, 균형에서 적극 적용, 적극에서 전면 적용한다(긴 좌향 수식과 대명사 밀도를 한층 공격적으로 분리).

이 등급 구분이 "보수를 고른 사용자에게 문장 전면 재구성을 강요"하는 모순을 막는다.

`translationese-patterns.md`의 축 설명에는 `과감`이 남아 있으나 그런 등급은 이제 없다. 그 문서는 upstream 분류 체계를 추적하는 대장이라 손대지 않는다. **의역 강도의 정본은 이 문서이며 등급은 보수, 균형, 적극 셋뿐이다.** 1:1로 일치해야 하는 것은 패턴 ID와 표면 교정형, 구조 재구성형의 대응이지 강도 목록이 아니다.
...
6. **줄표 조각.** 줄표에서 나눴는데 낱말이 되풀이되어 조각조각 느껴지는 곳이 있는가(예: `무엇이든 ... 무엇이든`)? "매끄러움" 패턴 D의 우선순위대로 괄호나 쉼표로 접어 본다.
...
12. **뻣뻣함 감사 (패턴 A~E).** "매끄러움" 절의 다섯 가지 뻣뻣함 패턴을 훑는다. **A, B, D, E는 어떤 톤과 강도에서도 검사하고, C만 톤 조건이 걸린다.** 격식 있는 기술 문서를 골랐다고 이 항목 전체를 건너뛰지 않는다.
    ...
    - (D) 산출물에 줄표(—)나 가운뎃점(·)이 남아 있는가? 코드, 인라인 코드, URL 밖이라면 패턴 D의 우선순위로 푼다.
