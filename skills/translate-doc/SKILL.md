...
21. **지어낸 비유 (density).** 번역본의 형상적 표현을 원문과 짝지어 훑는다. 대응하는 원문이 밋밋한 서술인데 한국어만 사물에 빗대거나 의인화한 자리가 있는가(`창구`, `앉는다`, `손잡이`, `밟는다`, `굴린다`, `그릇`)? 뜻이 흐린 동사(`깨진다`, `찬다`, `보수된다`)로 무엇이 어떻게 되는지를 감춘 자리는 없는가? 항목 2가 원문의 이미지를 **잃지 않았는지** 보는 자리라면 여기는 원문에 없는 이미지를 **더하지 않았는지** 보는 자리다. 처방은 Step 4 "원문에 없는 비유를 지어내지 않는다"를 따른다. 표면 교정형이므로 모든 의역 강도에서 적용한다.
