...
`references/ai-tell-rules.md`는 [im-not-ai](https://github.com/epoko77-ai/im-not-ai) 플러그인의 AI 티 패턴 목록에서 하우스 스타일이 이미 덮는 항목을 빼고 추렸습니다. `A`부터 `I`까지의 ID는 그쪽 체계를 따르고, `P`로 시작하는 과압축 묶음은 이 스킬에서만 쓰는 지역 ID입니다. 항목을 더하거나 빼도 스킬은 문제없이 실행됩니다.

과압축(`P`) 묶음은 나머지 규칙과 방향이 반대입니다. 군더더기를 덜어내다 조사와 어미까지 빠져 뜻이 흐려진 자리를 되돌립니다. 다만 되살리는 것은 문법적 표현까지이고 원문에 없는 정보는 채우지 않습니다. 채울 수 없는 자리는 "그대로 둔 곳"에 적어 돌려드립니다.
