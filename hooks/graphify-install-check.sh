...
#
# graphify 설정 = `graphify install --project --platform claude`가 {PROJECT_ROOT}/.claude/settings.json에
...
printf '%s' '{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"이 프로젝트에는 graphify 설정이 없습니다(.claude/settings.json에 graphify 훅 미등록). graphify는 Claude가 코드 질문에 답하거나 코드를 수정하기 전에 항상 지식 그래프를 먼저 참조하도록 만드는 도구입니다. 이번 세션의 첫 행동으로, 사용자의 다른 요청을 처리하기 전에, AskUserQuestion으로 이 프로젝트에 graphify를 설정할지 먼저 물어보세요. 비개발/문서 위주 프로젝트라면 필요 없을 수 있다고 함께 안내하세요. 사용자가 동의하면 프로젝트 루트에서 graphify install --project --platform claude 명령을 실행해 설정하세요. 사용자가 거절하면 설치하지 말고, 프로젝트의 .claude/ 디렉토리에 .graphify-skip 빈 파일을 만들어(.claude 디렉토리가 없으면 먼저 생성) 이후 세션에서 다시 묻지 않도록 하세요. 이 graphify 설정 질문을 먼저 끝낸 뒤에 사용자의 원래 요청을 처리하세요. 사용자가 다른 작업을 먼저 해달라고 명시적으로 요구할 때만 순서를 양보하세요."}}'
