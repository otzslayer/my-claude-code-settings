...
case "$skill" in
  ...
  *ce-plan)
    emit "필수: model·effort를 복잡도 채점(hybrid-workflow.md 3장)으로 산출해 먼저 announce하고 현재 세션과 다르면 전환 안내(강제 아님) — 계획 수립은 대개 개방형 추론(base 5) 이상이라 opus·high 이상 밴드일 가능성이 높다. Phase 2 PLAN: non-plan-mode에서 실행(Plan Mode가 ce-plan의 계획 파일 Write와 ce-doc-review autofix를 차단하므로). 병렬 리서치+CodeGraph 패턴 수집, docs/solutions/ 3개 이상이면 ce-learnings-researcher 조회. 계획 본문 산문은 반드시 한국어로 작성하라(코드·식별자·파일경로·frontmatter 키·enum 값은 영문 유지) — 병렬 리서치 서브에이전트 중간 산출물이 영어여도 최종 docs/plans/ 파일 Write는 한국어여야 한다. docs/plans/ 초안 Write → 자동 ce-doc-review(전면 opus, 5장) → plannotator annotate docs/plans/<파일>로 정본을 제자리 리뷰하라(브라켓·별도 파일 없음). 반환값: approved면 인라인 구현 금지·중단 후 /clear → 새 세션에서 /ce-work(opus·medium), annotated면 non-plan-mode에서 같은 정본에 피드백 반영 후 approved까지 재실행, dismissed는 승인 아님이므로 진행 금지." ;;
  *ce-doc-review)
