...
- Intelligence: fable > opus > sonnet. Cost: fable is the most expensive tier.
...
- **Boundary rounding rule** (concrete instance of §4's "round up at boundaries"): a pure base-5 task (open-ended reasoning, zero additive signals) lands on 5 — round up to opus·high, so open-ended reasoning never stays on sonnet.
...
- **Build-stage carve-out (ce-work / Phase 2')**: build against a finalized plan scores **base 1** (the plan already absorbed the design judgment). Planning-time signals the plan resolved — file count, new module/pattern, public-API/schema change — are **not re-counted** (re-counting double-prices scope the plan already paid for). Default is **sonnet·medium regardless of file count**; file count drives unit *volume* (see Unit granularity), not model tier. Build reaches opus only reactively via the §4 re-run gate, never pre-emptively on scope. Whenever a finalized plan exists, this settles the build-vs-"well-defined feature" ambiguity in favor of base 1.
