...
**Build-stage carve-out (ce-work / Phase 2')**: work executed against a finalized plan is scored **base 1** (mechanical execution — the plan already absorbed the design judgment), and the planning-time additive signals the plan resolved — file count, new module/pattern, public-API/schema change — are **not re-counted** for the build stage; re-counting them double-prices scope the plan already paid for. So a well-planned build defaults to **sonnet·medium** no matter how many files it touches — file count drives unit *volume* (handled by "Unit granularity" below), not model tier. Build reaches opus only reactively, via the §4 re-run gate — never pre-emptively on scope. (The base table already lists "build" at base 1; this note settles the build-vs-"well-defined feature" ambiguity in favor of base 1 whenever a finalized plan exists.)
...
- **Build-stage escalation is reactive, not scope-priced**: ce-work executes a finalized plan, so it starts at sonnet·medium (per the §3 build carve-out) and does **not** escalate on the plan's own scope — file count, new module, API/schema change are already priced into the plan. It escalates to opus only when execution surfaces what the plan did not foresee: a RED→GREEN stall that turns into open-ended root-cause debugging (that sub-task is re-scored as base-5 debugging, not build — matching the `systematic-debugging` = opus lineage), or a test/typecheck/verification failure on a hard-to-reverse change per the re-run gate above.
