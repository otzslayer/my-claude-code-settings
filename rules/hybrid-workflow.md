...
**Band** — every band is opus; the score sets `effort` only. Routing ceiling is opus·xhigh.
...
- **Build carve-out (§3)** — `/ce-work` against a finalized plan is **base 1** and planning-time signals are not re-counted → **opus·medium regardless of file count**. Escalate only reactively, never on plan scope.
- **fable-5** is never reached by score. Opt in only via an explicit long-horizon flag (sustained multi-subsystem design/implementation, long agentic chain). Default effort `high`.
